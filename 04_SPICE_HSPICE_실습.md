...
DC simulation은 $I_{D}-V_{G}$ curve처럼 정적인 특성을 보기 좋다. Transient simulation은 input pulse, load capacitance, delay, switching energy, aging stress처럼 시간 의존적인 현상을 보기 위해 필요하다.
...
Duty cycle은 transistor가 stress 상태에 머무는 비율이다. PMOS와 NMOS는 input 상태에 따라 stress를 받는 시간이 다르므로, input high time이 바뀌면 PMOS/NMOS의 aging 정도도 달라진다.
